function [x,f,exitflag,output] = minFuncProj( ...
  funObj, projFun, x0, options, varargin)
% minFunc(funObj,x0,options,varargin)
%
% Unconstrained optimizer using a line search strategy;
% the only constraints are realized by the projection
%
...
%   projFun is a function handle for the projection function
...
% Modified by Mateusz Malinowski (2012)
% Original version: http://www.cs.ubc.ca/~schmidtm/Software/minFunc.html
%
...
for i = 1:maxIter
    ...
    elseif LS < 6
        ...
        if method >= NEWTON
            [f_new,g_new,H] = funObj(projFun(x + t*d),varargin{:});
            funEvals = funEvals + 1;
        end
        x = projFun(x + t*d);
        f = f_new;
